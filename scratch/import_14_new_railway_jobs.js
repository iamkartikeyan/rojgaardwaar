...
async function run() {
  console.log('Searching for fresh Railway jobs in parallel from indgovtjobs.in...');
  const railwayQueries = [
    'Railway', 'RRB', 'RRC', 'Railway Apprentice', 'Railway Sports Quota', 
    'Metro Rail', 'DMRC', 'Maharashtra Metro', 'Konkan Railway', 'DFCCIL', 
    'RITES', 'Rail Coach Factory', 'CLW', 'Rail Wheel Factory', 'IRCTC', 'CRPF Railway'
  ];
  
  const queryResults = await Promise.all(railwayQueries.map(q => fetchQuery(q)));
  ...
  console.log(`Found ${candidateEntries.length} unique candidate Railway posts.`);

  // Pick top 14 Railway jobs
  const newRailwayEntries = candidateEntries.slice(0, 14);
  console.log(`Selected 14 fresh Railway recruitments:`);
  newRailwayEntries.forEach((item, idx) => {
  ...
  for (const item of newRailwayEntries) {
    const e = item.entry;
    ...
    let postName = vacMatch ? vacMatch[2].trim() : 'Railway Technical & Non-Technical Posts';
  ...
  console.log(`Successfully added 14 fresh Railway jobs! Total recruitments on site: ${combinedJobs.length}`);
}
