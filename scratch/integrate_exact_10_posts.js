...
console.log('Generating 10 exact articles from indgovtjobs.in...');

exactPosts.forEach((post, i) => {
  const filePath = path.join(__dirname, '../jobs', `${post.id}.html`);
...
<head>
  ...
  <title>${post.title} | RozgarDwaar</title>
  
  <meta name="title" content="${post.title} - RozgarDwaar">
  <meta name="description" content="${post.title} - Official Notification, Eligibility Criteria, Selection Process, and Apply Online Link on RozgarDwaar.">
  ...
  <link rel="canonical" href="https://rozgardwaar.com/jobs/${post.id}.html">
  ...
  <style>
    ...
    .exact-article-body table {
      width: 100%;
      border-collapse: collapse;
      margin: 16px 0;
      font-size: 13px;
    }
    .exact-article-body table, .exact-article-body th, .exact-article-body td {
      border: 1px solid #cbd5e1;
      padding: 8px 12px;
    }
    .exact-article-body th {
      background: #f1f5f9;
      font-weight: 700;
      color: #0b2545;
    }
    .exact-article-body h2, .exact-article-body h3, .exact-article-body h4 {
      color: #0b2545;
      margin-top: 20px;
      margin-bottom: 10px;
    }
    .exact-article-body ul, .exact-article-body ol {
      padding-left: 22px;
      margin-bottom: 14px;
    }
    .exact-article-body li {
      margin-bottom: 6px;
    }
    .exact-article-body a {
      color: #0066cc;
      font-weight: 600;
      text-decoration: underline;
    }
    .exact-article-body b, .exact-article-body strong {
      color: #000;
    }
  </style>
</head>
<body>
      ...
      <div class="main-two-col-layout">
        ...
        <div class="primary-content-column">
          <div class="content-block" style="padding: 20px;">
            ...
            <!-- Single Clean H1 Tag -->
            <h1 class="portal-main-h1">${post.title}</h1>
            <div style="font-size:12px; color:#555; margin-bottom:14px; border-bottom:1px solid #eee; padding-bottom:8px;">
              Published by: <strong>RozgarDwaar Editorial Desk</strong> | Updated: <strong>${post.publishedDate}</strong>
            </div>

            <!-- Action CTA Bar -->
            <div class="action-cta-bar">
              <a href="${post.applyUrl}" target="_blank" rel="noopener noreferrer" class="btn-cta-apply">
              ...
              <a href="${post.pdfUrl}" target="_blank" rel="noopener noreferrer" class="btn-cta-pdf">
  ...
  console.log(`Generated exact job file: jobs/${post.id}.html`);
});

console.log('Finished generating all 10 exact articles from indgovtjobs.in!');
