...
async function run() {
  console.log('Searching for fresh B.Tech / Engineering jobs in parallel from indgovtjobs.in...');
  const engQueries = [
    'Engineer', 'B.Tech', 'B.E.', 'Executive Engineer', 'Assistant Engineer', 
    'Scientist Engineer', 'ISRO Engineer', 'DRDO RAC', 'BARC Scientific Officer', 
    'NPCIL Executive Trainee', 'NTPC EET', 'THDC Engineer', 'SJVN Engineer', 'MIDHANI Engineer'
  ];
  
  const queryResults = await Promise.all(engQueries.map(q => fetchQuery(q)));
  ...
  console.log(`Found ${candidateEntries.length} unique candidate B.Tech / Engineering posts.`);

  // Pick top 12 Engineering jobs
  const newEngEntries = candidateEntries.slice(0, 12);
  console.log(`Selected 12 fresh Engineering recruitments:`);
  newEngEntries.forEach((item, idx) => {
  ...
  for (const item of newEngEntries) {
    const e = item.entry;
    ...
    let vacCount = vacMatch ? parseInt(vacMatch[1].replace(/,/g, ''), 10) : 40;
    let postName = vacMatch ? vacMatch[2].trim() : 'Project Engineer / Executive Trainee (Engineering)';
    ...
    const dbJob = {
      ...
      qualifications: ["graduate", "btech-engineering", "diploma"],
      category: "psu",
  ...
  console.log(`Successfully added 12 fresh B.Tech / Engineering jobs! Total recruitments on site: ${combinedJobs.length}`);
}
