...
async function run() {
  console.log('Searching for fresh Medical / MBBS / Nursing jobs in parallel from indgovtjobs.in...');
  const medicalQueries = [
    'Medical Officer', 'MBBS Doctor', 'Staff Nurse', 'Nursing Officer', 
    'Pharmacist', 'BDS', 'Dentist', 'AIIMS Nursing', 'ESIC Doctor', 
    'RUHS Medical', 'Health Department', 'GDMO', 'Veterinary Pharmacist'
  ];
  
  const queryResults = await Promise.all(medicalQueries.map(q => fetchQuery(q)));
  ...
  console.log(`Found ${candidateEntries.length} unique candidate Medical / Nursing posts.`);

  // Pick top 13 Medical jobs
  const newMedicalEntries = candidateEntries.slice(0, 13);
  console.log(`Selected 13 fresh Medical recruitments:`);
  newMedicalEntries.forEach((item, idx) => {
  ...
  for (const item of newMedicalEntries) {
    const e = item.entry;
    ...
    let postName = vacMatch ? vacMatch[2].trim() : 'Medical Officer / Staff Nurse / Pharmacist / Specialist Doctor';
    ...
    const dbJob = {
      ...
      qualifications: ["graduate", "medical-nursing"],
  ...
  console.log(`Successfully added 13 fresh Medical jobs! Total recruitments on site: ${combinedJobs.length}`);
}
