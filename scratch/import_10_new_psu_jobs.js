...
async function run() {
  console.log('Searching for fresh PSU jobs in parallel from indgovtjobs.in...');
  const psuQueries = ['NTPC', 'BHEL', 'ONGC', 'SAIL', 'POWERGRID', 'BPCL', 'HPCL', 'HAL', 'BEL', 'BEML', 'NMDC', 'THDC', 'SJVN', 'MIDHANI', 'RCFL', 'NSPCL', 'MRPL', 'SECI', 'HURL', 'GIPCL'];
  
  const queryResults = await Promise.all(psuQueries.map(q => fetchQuery(q)));
  ...
  queryResults.forEach(entries => {
    entries.forEach(e => {
      ...
      if (/opening|closing|sarkari|digest|jobs 2026 \(|pass govt jobs|admit card|result/i.test(title)) return;
  ...
  console.log(`Found ${candidateEntries.length} unique candidate PSU posts.`);

  // Pick top 10 PSU jobs
  const newPsuEntries = candidateEntries.slice(0, 10);
  console.log(`Selected 10 fresh PSU recruitments:`);
  newPsuEntries.forEach((item, idx) => {
  ...
  for (const item of newPsuEntries) {
    const e = item.entry;
    ...
    let vacCount = vacMatch ? parseInt(vacMatch[1].replace(/,/g, ''), 10) : 12;
    let postName = vacMatch ? vacMatch[2].trim() : 'Project Engineer / Executive Trainee';
  ...
  console.log(`Successfully added 10 fresh PSU jobs! Total recruitments on site: ${combinedJobs.length}`);
}
