...
function cleanExactContent(html) {
  ...
  // Replace references to indgovtjobs with rozgardwaar
...
async function run() {
  console.log('Fetching top 10 exact articles from indgovtjobs.in feed...');
  const json = await fetchJson('https://www.indgovtjobs.in/feeds/posts/default?alt=json&max-results=10');
  const entries = json.feed.entry;

  console.log(`Retrieved ${entries.length} exact posts from indgovtjobs.in.`);
  ...
  console.log('Saved exact 10 posts to scratch/exact_10_posts.json!');
}
