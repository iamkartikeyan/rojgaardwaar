...
// Map state name from query or title
function detectState(title) {
  const t = title.toLowerCase();
  if (/up|uttar pradesh|upsssc|uppsc/i.test(t)) return 'uttar-pradesh';
  if (/bihar|bpsc|bssc/i.test(t)) return 'bihar';
  if (/rajasthan|rsmssb|rpsc|rssb/i.test(t)) return 'rajasthan';
  if (/maharashtra|mpsc|maha/i.test(t)) return 'maharashtra';
  if (/delhi|dsssb/i.test(t)) return 'delhi';
  if (/haryana|hssc|hpsc/i.test(t)) return 'haryana';
  if (/tamil nadu|tnpsc|tneb/i.test(t)) return 'tamil-nadu';
  if (/kerala|kpsc/i.test(t)) return 'kerala';
  if (/karnataka|kea/i.test(t)) return 'karnataka';
  if (/gujarat|gpsc|gsssb/i.test(t)) return 'gujarat';
  if (/punjab|psssb|ppsc/i.test(t)) return 'punjab';
  if (/west bengal|wbpsc|wbprb/i.test(t)) return 'west-bengal';
  if (/odisha|osssc|opsc/i.test(t)) return 'odisha';
  if (/telangana|tspsc|tslprb/i.test(t)) return 'telangana';
  if (/andhra|appsc|ap/i.test(t)) return 'andhra-pradesh';
  if (/mp|madhya pradesh|mpesb|mppsc/i.test(t)) return 'madhya-pradesh';
  if (/uttarakhand|uksssc|ukpsc/i.test(t)) return 'uttarakhand';
  return 'all-india';
}
...
async function run() {
  console.log('Searching for fresh State Govt jobs in parallel from indgovtjobs.in...');
  const stateQueries = [
    'UPSSSC', 'BPSC', 'RSMSSB', 'TNPSC', 'MPSC', 'HSSC', 'UKSSSC', 'WBPSC', 
    'TSLPRB', 'OSSSC', 'GPSC', 'DSSSB', 'PSSSB', 'APPSC', 'TNPCB', 'KPSC', 'RPSC'
  ];
  
  const queryResults = await Promise.all(stateQueries.map(q => fetchQuery(q)));
  ...
  console.log(`Found ${candidateEntries.length} unique candidate State Govt posts.`);

  // Pick top 10 State Govt jobs
  const newStateEntries = candidateEntries.slice(0, 10);
  console.log(`Selected 10 fresh State Govt recruitments:`);
  newStateEntries.forEach((item, idx) => {
  ...
  for (const item of newStateEntries) {
    const e = item.entry;
    ...
    let vacCount = vacMatch ? parseInt(vacMatch[1].replace(/,/g, ''), 10) : 50;
    let postName = vacMatch ? vacMatch[2].trim() : 'Various State Cadre Vacancies';

    let dateMatch = item.title.match(/Last\s*Date\s*[:\s]*([0-9]{1,2}[-/][0-9]{1,2}[-/][0-9]{4})/i);
    let lastDate = dateMatch ? dateMatch[1] : '2026-09-30';

    const stateKey = detectState(item.title);
    ...
    const dbJob = {
      ...
      qualifications: ["graduate", "12th-pass", "10th-pass"],
      category: "state-govt",
  ...
  console.log(`Successfully added 10 fresh State Govt jobs! Total recruitments on site: ${combinedJobs.length}`);
}
